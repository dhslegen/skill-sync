...
/// 保守起见按同等的"脏数据"处理。
///
/// **`source` 形状不像 `owner/repo` 也一律跳过**(2026-08-19 补,判据见
...
///
/// **返回 `Option<(&str, &str)>` 而不是 `bool` 是 2026-08-19 审查的结论**:
...
///
/// 这道过滤分两次落地,**日期不同,别当成同一天的事**(下面一律只标日期不编号
/// ——"第几审"与"第几轮"曾经错位一位,同一件事在本文件里得出两个日期,
/// 正是 2026-08-19 复审抓到的问题):
/// - **2026-08-17**,只修了排行榜([`to_leaderboard_card`])。起因:`plaza_detail` 拿到
...
///   不摆比解释好)。
/// - **2026-08-19**,补上搜索([`to_card`]):点开搜索结果的失败机制与点开排行榜条目
