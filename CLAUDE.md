...
- **本期(2026-08-24 至 08-29)实测确认的、与本身设计无关的既有小缺口,如实登记
  ...
  ②**「在技能库里查看」未进「…」菜单**(R20):`InstalledSkillView` 里唯一的库 URL
  是 `review.url`,只在"审核中"那档有值;要按设计 §8 摆进菜单需扩 DTO,超出 v7 边界;
  ③**项目行的工具落点路径留空**:`DetectedAgent` 只有全局的 `globalSkillsDir`,
  穿到项目行会撒谎;补法是让它暴露相对 `skillsDir`;
