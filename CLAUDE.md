...
**下一个里程碑 = M11(工作代号「v5 开发」),2026-08-20 用户指定,两条目标**
...
2. **应用更新后,更新日志首屏显示;其他情况版本日志收敛到设置里**。
   ...
   版本历史都从它来);`core/app_update.rs` 的 `ReadyState` 记着"装好等重启"这件事,
   但**它在进程内、重启即作废**——"刚升级完"这个判定要用什么做依据,是这条的第一个
   设计问题(别默认沿用 ReadyState)。
