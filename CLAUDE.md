...
- **本期(2026-08-24 至 08-29)实测确认的、与本身设计无关的既有小缺口,如实登记
  ...
  只收展示名;
  ⑤~~**`ToolPicker` 钉序没有内在防护**~~(R18)——✅ 0.6.x 已清:钉住的顺序连同
  agent 集合的指纹一起记,**集合变了就重钉**;⚠️ 它防不了"同一集合、换了一个
  技能"(指纹相同),那一半仍靠调用方在换技能时换组件实例,组件文档如实写着。
...
- **我的技能三区模型(v7)**——「我的技能」整页重画,动 `ownership.rs::in_builtin_library` /
  ...
  - **多选组件四处统一**(`ToolPicker.tsx`,`ToolPickerItem` 是唯一的入参形状;
    ...
    **这是一处经用户看截图批准的画布偏离**:画布 §15 给确认条画的是 `list`,
    ...
    组件注释与测试里钉住。0.6.x 起钉住的顺序带 agent 集合指纹,**集合变了才重钉**
    (同一实例被喂另一个技能的数据时不再按旧顺序排);集合恰好相同时不重钉,
    "换技能 = 换实例"这条仍是调用方的责任。
