...
- **全屏档(0.6.x 补,Demo 没画)**:内容区**不封顶、不居中**,左锚定铺满,顶栏全出血;
  设置页维持自己的 `max-w-[620px]` 靠左。Demo 的 `max-width:980px` 是 ~1200px 窗口下的值
...
- 🔴 **「注入变红了,但红的原因不是你以为的那个」——v6 二期复现了七次,v7 又至少
  复现一次(任务书明确编号「第八次」,见 v7 任务 2 的再复审:候选闸与尺子被一起
  改掉,红的红了却全部可以归因于键的改变而非闸本身;修法是把对照行的名字换成
  一条**只有闸缺席才会命中**的既有 PR),是本项目现役第一大陷阱**。七种形态:
  ①注入打在**另一个表达式**上(判据真身是
...
- 🔴 **「错误被写进某个状态,但没有渲染点」——v6 二期三次现身,v7 任务 6 是第四、
  第五次,第 6 次的落点是 v7 终审修复新摆的详情面板动作区**:
  失败写进 `useInstall.error`,而它唯一的渲染点是**不在场的** `InstallPanel`;
  `.catch(() => {})` 把 core 守卫的拒绝整个吞掉,用户点了「打开文件夹」什么都不会发生。
  v7 任务 6 的第四次现身:详情面板「各个工具里」摆了可点的勾,`setAgents` 的三种失败
  却只写进 store、唯一渲染点在**被遮罩盖住/根本没挂载**的 `MySkillsPage`,用户看到的是
  勾自己弹回未选中、零错误零提示。**判据一句话:每加一个错误出口,问一句"它失败的时候,
  用户在哪看到"。** jsdom 测得过是因为 mock 掉了 invoke,永远拿不到那个拒绝——这类只有
  真机点得出来。⚠️ **修复自己也会引入这条的变体**:v7 任务 6 修复轮 1 补齐渲染点后,
  修复轮 2 复审又抓到"跨技能状态泄漏"——失败字段是全局的、无归属校验,详情面板 B
  会显示技能 A 的失败,**从"零反馈"变成"错误的反馈"**,同样是对用户撒谎(关于*哪个*
  技能出了问题)。补错误出口时不能只问"有没有地方显示",还要问"显示的那句话点名的
  是不是这一个技能"。
  🔴 **第 6 次现身(v7 终审修复 §12,而且是上一笔提交自己放大的)**:设计 §12 把
  行上的「贡献更改」「分享」「移除」整套动作**搬进了详情面板的动作区**
  (`SkillActionsBlock`),却没把反馈一起搬——`shareDone`/`shareError` 的唯一渲染点
  仍是 `MySkillsPage`,而那一页从商店/广场详情打开时**根本没挂载**、从「我的技能」
  打开时**被全屏遮罩盖住**。「贡献更改」这一档最重:core 刻意不动记账,刷新后按钮
  原样还是「贡献更改」,用户看到的是"点了没反应",**再点一次就开出第二个内容相同
  的合并请求**(`share.rs::review_branch` 的分支名带时间戳)。
  **新增两条判据**:①**把一个动作搬到新地方时,同一笔改动里就要问"它的成功与失败
  在这个新地方由谁渲染"**——搬动作是重复既有缺陷最快的方式;②**成功也要有渲染点**,
  不只是失败:动作不改变任何可见状态时("记账一个字不动"),没有成功反馈就等于
  没反应,而"没反应"会诱发重复提交。
...
- 🔴 **「在已知假红清单里」是排查的起点,不是跳过的理由**(v6 二期终审修复波实测):
  ...
  `name: 冲突实测` + 文件夹名 `conflict-live-*`,**标准两条都犯**。改 fixture 时写明了
  "不是迁就守卫,回推这种技能今天本来就不该成功"。它留下的残留技能又打红了下一轮
  `gitea_live` 的清单断言——**那条才是真正的已知假红**。
  **一份"已知假红清单"最危险的用法就是拿它当跳过的理由:同一条测试今天红的原因可能与上次无关。**

- 🔴 **"谁改的"要自己核,子代理的自述与现象都不是证据**(v6 二期真实事故):
  终审者撞额度中断后被恢复,看到工作区里有大量改动,把它归因成"自己派的只读 fork 越权修改"
  并写进报告;协调者**只核了现象(工作区确实脏)、没核归属**,就写进台账、还向正在干活的
  agent 发了警告。实际那些文件全是修复波的正常产出——**铁证是时间戳**(新文件 21:21/21:29,
  而终审报告的 mtime 是 22:02),而且**协调者自己在派修复波之前跑过 `git status`,当时是干净的**
  ——**那条证据当时就有,却没在下判断时用上**。
  连带后果:终审报告里三条**真发现**被记成"自我证伪的假发现"(因为它读到的是已被修复的文件)。
  **判据:归属问题用时间戳与自己先前的观测记录裁定;转述"另一个 agent 做了什么"之前先自核。**
  本项目归纳的「声称的范围大于实际」,这次发生在协调者自己身上。
...
- 🔴 **官方文档已查实(2026-08-24),上面整段的判据要按这个重读**:
  - **Claude Code 里 frontmatter `name` 不参与调用**。官方原文:`name` 字段
    **`Required: No`**、「Display name shown in skill listings. **Defaults to the
    directory name.**」;「**In a personal or project skill, `name` sets only the
    display label shown in skill listings, and the command still comes from the
    directory name.**」;「The **directory name becomes the command you type**」。
    **只有插件技能例外**(那一档 `name` 决定命令名末段)。
    出处 <https://code.claude.com/docs/en/skills>。
  - **Agent Skills 开放标准要求两者必须一致**:`name` 为 **Required**,且
    「**Must match the parent directory name**」,字符集限
    **lowercase a-z / 0-9 / 连字符**、不得首尾连字符、不得连续连字符。
    出处 <https://agentskills.io/specification>。Claude Code 官方文档自称遵循该标准并做了扩展。
  - **两者冲突的解读**:Claude Code 比标准宽容,放行了 `name ≠ 目录名`(本机
    `react-best-practices` / `name: vercel-react-best-practices` 实测确实加载);
    但那样的技能**不符合标准**,别的遵循标准的工具不保证认。
  - **所以"本 app 用目录名"这个既有决策是对的**,而且理由比原先记的更硬:目录名是
    调用名的唯一来源,按目录名装能保证全公司调用名一致;按 frontmatter `name` 则是
    "作者改一个字段就改变所有人叫它的名字"。
- 纯中文名会被 `sanitize_name` 整体折成 `unnamed-skill`,两个中文技能会装进同一目录互相覆盖。
  installer 对"信息全丢"的名字报 `FS_UNUSABLE_NAME` 拒绝,**不放宽 `sanitize_name`**
  (它同时决定 `.skill-lock.json` 的键)。
  - ⚠️ **原「中文名技能分享策略」(M1 任务 11:表单强制起 ASCII 远端目录名、frontmatter
    `name` 保持中文显示名、本地目录一律不改名)已于 2026-08-24 被用户推翻**,原因是它
    **本身就违反标准**——`name` 只能是 ASCII 小写且必须等于目录名,中文 `name` 两条都犯。
    新口径见「待处理」的 v6 二期:分享环节零编辑 + 按标准全量校验,不合格不让分享,
    改名由用户在本地自行完成。`share.rs` 模块头与 `share.rs::adopt_into_management`
    的相关注释**尚未改**(实施时一并改),别照着它们写新代码。
...
- 🔴 **项目级有几组工具共用同一个 `skillsDir`,界面必须合并成一个勾**(2026-09-14 真机走查):
  ...
  真机复验又抓出两条,都是"组的身份随成员进出而变":组 id 必须**按目录定**(不能由成员
  拼成,否则一点就重排);标签**只取探测到的成员**(本机没装的 Trae 只因已关联才进组,
  进标签的话勾上叫「Trae CN、Trae」、取消后叫「Trae CN」,且与确认条对不上)。
...
## 当前进度(2026-09-24,现役 **v0.7.1** —— 当天发布)

**v0.7.1 已发布**(2026-09-24,curl 核实:内网 `v0.7.1` 五个产物齐全;`latest` version 0.7.1、三平台条目
均带签名;版本号提交 `095b5a7` 的 CI macOS/Windows 两 job 均 success)。内容 = 品牌图标 + `⌘N` 与侧边栏对齐。
发版前按「真机验收:agent 自主完成」在正式签名测试包上看了程序坞图标与 `⌘1/3/4`;Rust 源码自 0.7.0 零改动,
full 与门控 live 沿用 0.7.0 当天的结果。登记一条既有行为(未改):商店/广场详情面板在 `⌘N` 切页后仍盖在新页上。

(以下是 v0.7.0 当时的记录)


**v0.7.0 已发布**(2026-09-24,curl 核实:内网 `v0.7.0` 五个产物齐全;`latest` version 0.7.0、
三平台条目均带签名;版本号提交 `2083d4d` 的 CI run 35963014866 macOS/Windows 两 job 均 success)。
...
## 当前进度(2026-09-15,现役 **v0.6.3** —— 当天发布)

**v0.6.3 已发布**(2026-09-15,curl 核实:内网 `v0.6.3` 五个产物齐全;`latest` version 0.6.3、
三平台条目均带签名、下载 200;版本号提交的 CI run 34953089836 macOS/Windows 两 job 均 success)。
内容 = 全屏布局:去掉内容区 980 封顶、左锚定铺满(见「UI 规范」全屏档)。纯前端改动;发版前
手动跑了四条门控 live(真发请求)+ terminology 14/14;Rust full 未跑——0.6.2 之后 `src-tauri`
只动了 Cargo.lock,双平台测试由脚本的 CI 闸兜住。

(以下是 v0.6.2 当时的记录)


**v0.6.2 已发布**(2026-09-15,curl 核实:内网 `v0.6.2` 五个产物齐全;`latest` version 0.6.2、
三平台条目均带签名、下载 200;版本号提交 `9570c45` 双平台 CI 逐 job 绿)。内容 = Windows 项目内
工具勾不上(verbatim 路径前缀)+ keyring 首次使用并发竞态。发版前走了三轮 Windows CI:
①keyring 竞态挡住了 fail-fast,新测试没跑到;②竞态修好后露出 786be12 自己打红的三条
project_registry(存 normalize 结果把 `/w/a` 改成 `\w\a`);③全绿。**两次红都是 macOS 本机恒绿的形状。**
⚠️ **Windows 真机复验仍欠**:勾选修复只在 CI 里验过,用户那台机器上的「项目里的技能」要升到 0.6.2 后看一眼。

(以下是 v0.6.1 当时的记录)

**v0.6.1 已发布**(2026-09-14,curl 核实:内网 `v0.6.1` 五个产物齐全;`latest` 公告牌
version 0.6.1、三平台条目均带签名、下载链接 200;版本号提交 `a72100d` 双平台 CI 逐 job 绿)。
内容 = 0.6.x 走查修复:按页刷新语义 + 5 分钟只读兜底 + 切到商店也刷新、项目内共用目录的
工具只显示一个勾、项目技能本体缺失态、换机后第 4 源详情、K3–K5。发版前 Rust full
904 passed / 1 failed(发版说明术语守卫抓到「合并」,改后 14/14)/ 3 ignored,四条门控 live 手动真跑。

(以下是 v0.6.0 当时的记录)

**v0.6.0 已发布**(2026-09-11,逐项 curl 核实过终态,不是照抄脚本输出):内网发布仓
`v0.6.0` **五个产物齐全**(dmg 18.8MB / x64-setup.exe / exe.sig / app.tar.gz /
app.tar.gz.sig);`latest` 公告牌 **三平台条目齐全且各自带签名**
(darwin-aarch64 / darwin-x86_64 / windows-x86_64),version 字段为 `0.6.0`;
macOS 包已签名·已公证·票据已装订;README 与 RELEASE_NOTES 已同步到发布仓。
内容 = **v6 一期 + v6 二期 + v7 + v7.1–v7.7**(此前四节里所有「尚未发版」「发版前
还欠」的说法都已作废,保留在下面只作为各阶段的历史记录)。

**v7.1–v7.7 = 发版前的七轮真机驱动修复**(2026-09-09 至 09-11,用户在真机上逐轮
指出问题,设计与共识在本地 `docs/v7.x-共识.md`,不受版本控制)。它们全部随 v0.6.0
发出,机制约束见「现役机制约束」对应各节:
- **v7.1–v7.3**:表格渲染、三区改页签、角标收敛、按钮主次分级、侧边栏图标统一。
- **v7.4**:撤掉商店卡片的作者四档(「取回」一词背三种意思、「已同步」回答没人问的
  问题);删档不删机制,core 的作者保护四件一件没动。
- **v7.5 + v7.6**:**磁盘是真相,记账只回答"从哪来"**——整条拍板见
  「现役机制约束」同名一节。
- **v7.6 任务 2–3**:详情面板两条渲染路径同构成一个固定页脚;「在哪」补第四块
  「项目里」;确认条竖排 + picker 改 `inline`(经用户看截图批准的画布偏离)。
- **v7.7**:两个浮层菜单(`SkillRowMenu`/`InstallScopeMenu`)改 **portal 到
  `document.body` + `fixed` + 按触发器真实 rect 自动翻转**,定位逻辑共用
  `src/hooks/useFloatingMenu.ts` + 纯函数 `src/lib/floating-menu-position.ts`。
  起因:`absolute` 浮层会被祖先 `overflow-hidden` 裁掉(「我的技能」列表卡片),
  或画到视口外且不跟着滚(项目行底部行)——**四个宿主里三个坏**。
  🔴 portal 的三个坑已各自钉住:外点关闭要同时查 anchor 与 menu 两个 ref;
...
自己分段拼出来的是 `...\.claude\skills\x`——**同一个目录,字符串却不等**,
凡是拿 fixture 路径去和账上字符串比的断言全红(`tests/acquire_flow.rs` 4 条,
`tests/installed_list.rs` 同源但因 **`cargo test` 是 fail-fast 的**根本没跑到)。
- **是测试侧缺陷,产品不受影响**:core 产出的是规范路径,错的是 fixture 的期望值;
  已发出的 0.6.0 包没有这个问题。
- 修法是在 **fixture 构造那一层根治**:两个文件各加一个 `join_rel(base, "a/b")`
  分段 join,让测试数据本身就是规范路径——比逐个改断言更不容易漏。
- ⚠️ **macOS 上两种写法恰好相同,本机怎么跑都是绿的**。这条只有 Windows CI 拦得住,
  与本文件既有的「本机 clippy 绿 ≠ 另一个平台绿」是同一条。

🔴 **发版当天的两条事实,下一个会话必须知道**:
1. **发版过程被中断过一次,已恢复**:第一次跑 `publish-release.sh 0.6.0` 时
   tag 已推、macOS 构建中途被杀。处置是先查「latest 公告牌动没动」——它排在脚本
   最后,是"全公司看得到新版本"的**唯一开关**,没动就说明没有半成品到达用户。
   脚本本身可重入(第 23 行写明「版本号 commit 与 tag 已推也无妨,重跑会沿用」,
...
3. 🔴 **发版当时 `CI` workflow 在 Windows job 上红了,而包已经发出去了**——因为
   **`publish-release.sh` 等的是 `Release` workflow(要它的 exe artifact),不等
   ...
   ✅ **v0.6.1(2026-09-14)是这道闸的第一次真跑**:按 headSha 等到版本号提交
   `a72100d` 的 CI(run 34835528624,macOS/Windows 两 job 均 success)后才写内网。
   ⚠️ 同一次真跑暴露了脚本末尾 `[[ -n "$SKIP_CI" ]] && echo` 让**成功的发版以退出码 1
   结束**(条件不成立返回 1,恰是最后一条命令),已改成 `if`。**发版结论一律 curl 核终态**,
   退出码与脚本自报都不算数。


**v7 = 「我的技能」重设计**(2026-08-28 拷问式访谈拍板,2026-08-29 完成,设计在本地
`docs/设计-v7-我的技能重设计.md`,任务分解 `docs/v7-任务分解.md`,均不受版本控制)。
起因是用户提的七条真实缺陷:Zed 被误显示成「本体在这里」、「我分享的」区混进了从未
...
评审 → 任务 5 多选组件三处统一 → 任务 6 详情面板「在哪」三块 → 任务 7 我的技能整页
重画(三区 / 只写例外 / 行尾菜单)→ 任务 8 项目页签 + 项目级工具改选 →
任务 9(本任务)端到端 + 承诺同步。详见「现役机制约束」的「我的技能三区模型(v7)」
一节。**并入 0.6.0 一起发**(用户拍板:用户第一次看到「我的技能」就该是新版,
不必先经历一次"我分享的/我安装的"那套已经要撤销的说法),`RELEASE_NOTES.md` 的
0.6.0 段落已按 v7 重写。
...
- 🔴 **任务书叙述与已拍板设计有两处对不上,本任务按设计文档走,证据链写进了
  `tests/e2e_sections.rs` 文件头**(与本期 Task 1 的 `trae-cn` fixture、Task 3 的
  `Foreign` 档缺测试、Task 8 的"动后端"同一种处置:实现者发现任务书本身的缺陷,
  按更权威的来源——已拍板的设计文档——纠正,并显式写明原委,交复审核验):
  ①**"审核中"是「可分享到」区专属,不适用于「安装自」区的「贡献更改」**——
  `docs/设计-v7-我的技能重设计.md` 决策 #4/#7 与 `docs/v7-任务分解.md` 第 33 行
  (「贡献更改」不新造路)都明确把"审核中"限定在「可分享到」区,现有代码
  (`my_skills::has_review_candidates` 等三处)也确实只对该区生效;
  ②**"贡献更改走评审后合并,回到安装自·有更新"这个终态在当前实现下不可达**,
  真实终态是"库里有新版…"三选一冲突档(`RowAction::conflict`)——贡献更改走评审时
  刻意不更新 `content_hash` 基线("记账一个字不动"),PR 合并后本地与库内容相对
  这个旧基线**同时**变了,`rowAction` 的短路顺序让 `localModified && remoteChanged`
  先于单纯 `remoteChanged` 命中。见「我的技能三区模型(v7)」一节的完整原委。
- ~~**发版前还欠(与 v6 两期的清单合并)**~~ **——已随 v0.6.0 发布作废,以下是当时的清单**:①本地**未推送**,没有 CI 记录;
  ②**真机视觉走查未做**——「我的技能」整页三次重画、多选组件三处、项目页签、
  未登录、审核中那一行、故意断网看审核态降级,全部只在 jsdom / wiremock 里验过;
  ③`plaza_live` / `plaza_blob_live` / `plaza_install_blob_live` / `trash_live`
  四条门控 live 测试本任务未手动跑,发版前必须按「发版」一节手动跑一次;
  ④**整分支终审未做**——v6 两期的经验是逐任务审查零 Critical、终审各抓到 3 条,
  本期同样可能有跨任务接缝(检查点见 `docs/v7-任务分解.md`「整分支终审」一节)。
- **本期(2026-08-24 至 08-29)实测确认的、与本身设计无关的既有小缺口,如实登记
  不在本期修**:
  ①**Windows 上降级复制的僵尸副本**(任务 3 的 I4 后半,R15 裁定推迟):junction
  建不成时降级复制,`current_agents` 认不出它 → 更新时该工具从名单消失 → 副本从此
  与本体不同 → 取消勾选落 `kept`(删不掉)、重新勾选撞 `FS_LINK_OCCUPIED` 也落
  `kept`(接不回)→ 永久僵尸副本,应用内零出路,只在 Windows 可达;
  ②~~**「在技能库里查看」未进「…」菜单**~~(R20)——✅ v7.6 收尾 2(`17fd696`)已清:
  DTO 扩了 `libraryUrl`(`my_skills::library_url`),「…」里有它;这条一直没划掉,
  0.6.x 核对 K 列时才发现;
  ③~~**项目行的工具落点路径留空**~~——✅ 0.6.x 已清:`DetectedAgent` 新增
  `skillsDir`(注册表相对目录,与 `project::link_dirs` 建链用的是同一个字段,
  所以显示的落点 = 真实落点),项目行 picker 拼 `<项目>/<skillsDir>` **仅供展示**
  (Windows 上是混排,别拿它比对);确认条(`ConfirmBar`)仍刻意留空,见下面
  「多选组件四处统一」;
  ④~~**`SkillRowMenu` 的多个「更多」共享同一 aria-label**~~(任务 8 的 M5)
  ——✅ 0.6.x 已清(`ed3d221`):可访问名改成「{name} 的更多操作」,`subject` 必填且
  只收展示名;
  ⑤~~**`ToolPicker` 钉序没有内在防护**~~(R18)——✅ 0.6.x 已清:**新 items 与钉住的
  顺序毫无交集(整批换掉)才重钉**。⚠️ 第一版按"集合指纹变了就重钉",真机复验当场
  回归:项目行取消一个未探测到的工具,它从列表消失、集合变了,刚点的那一项被甩到末尾
  ——违反"操作期间绝不重排"。它防不了"换了技能但工具有交集"(候选对每个技能都一样,
  是常态),那一半仍靠调用方在换技能时换组件实例,组件文档如实写着。
...
`localDiffers`/`versionChoice` + 移除单确认 → 任务 9 端到端作者闭环 +
术语门第四条通道 + 承诺同步 → **整分支终审 + 一次修复波**(`1db3f87`,见下)。
详见「现役机制约束」的「本体与位置模型(v6 二期)」一节。
**v6 一期与二期一起发 0.6.0**(用户拍板:两期是同一个问题,分开发会让用户先在错误的
设计下用一遍),`RELEASE_NOTES.md` 的 0.6.0 段落已按两期重写。

**v6 二期的终态与欠账(2026-08-27 接力会话实测,别照抄下一份文档)**:共 **28 个提交**
(`1672766..` 至「复审轮1」那一笔):终审修复波 `1db3f87` + 洁癖收尾 `1d70a48` + 修复波的定向复审轮(标题「v6二期-终审修复 复审轮1」)。

🔴 **整分支终审的价值第二次兑现**(v6 一期抓到 3 条,这次又是 3 条):**九轮逐任务审查
零 Critical,终审当场抓到 3 条 Critical + 3 条 Important**,而且三条 Critical **形状完全相同**——
**core 把事实准备好了、上了 IPC、前端也有类型,就是没有一处代码去读它**:
`KeepReport` 的失败出口(→ 版本拍板在废纸篓被拒时是**零反馈死循环**)、
三句「无法找回」(实际走废纸篓,**可以找回**;而同文件 50 行外的注释正指着它说"别照抄,那是假话")、
`shareBlocked` 的校验结论(→ `share_installed` 与三颗分享按钮**整条绕过 A-2 校验闸**,
旗舰场景下能把不合格技能**直推进库**)。
**结论:逐任务审查在结构上看不见接缝——它们交付的是同一件事的两半,而两半之间没有人对过表。**
接缝表其余部分零缺格(`Precheck` 八档 × 五层、`SetAgentsOutcome` 四个错误出口、六态机八档 × 两分区)。

- ~~**发版前还欠两件**~~ **——已随 v0.6.0 发布作废**:①本地**未推送**,所以**没有 CI 记录**——按项目
  ...
  ✅ 第三件「修复波的定向复审」已于 2026-08-27 做完(「复审轮1」那一笔):抓到**第四句**「找不回来」
  (`conflict.otherLibraryBody`,与同屏 hint 自相矛盾)——C-2 自己配的负向断言只查
  `/无法找回/`,抓不住变体。**教训:钉一句假话时,负向断言要按语义写(`/无法找回|找不回/`),
  不能只抄被改掉的那个字面量**;而且「把同一句话的所有出现位置列出来」这一步要在改之前做。
...
详见「现役机制约束」的「技能归属模型(v6)」一节。**已随 v0.6.0 发布(2026-09-11)**,
`RELEASE_NOTES.md` 的 0.6.0 段落已写好。

**v6 的终态与欠账(2026-08-24 实测,别照抄下一份文档)**:共 **19 个提交**
(`10b29c5..8c18262`),Rust **739** / 前端 **643** 全绿(docker fixture 起着、
`gitea_live` 真跑),clippy `--all-targets` / tsc / eslint 干净,工作区干净。
- 🔴 **整分支终审抓到 3 条 Critical,六轮逐任务审查一条都没发现**,因为三条全部跨任务:
  ①`share::precheck` 没接 `ownership::relation`,用户点名的"第一作者直推 git"场景走到
  那里仍弹「而它不是你分享的」——**v6 要消灭的那句话在另一个函数里原样活着**;
  ②`install.ts` 的过滤器换写法时丢了等价性,npx 装过的同名技能让商店卡片显示禁用的
  「已启用」;③`Row` 无条件渲染「移除」,无记账行点了先弹"将删除本地文件"再报错。
  **这是"逐任务审查看不见接缝"最贵的一次复现,别因为每个任务都绿就跳过整分支终审。**
- ~~**发版前还欠三件**~~ **——已随 v0.6.0 发布作废**:①本地**未推送**,所以**没有 CI 记录**——按项目
  铁规,`gh run list` 逐 job 核过双平台之前不得写「CI 绿」;②「我的技能」整页重画过,
  **真机视觉走查未做**;③场景「第一作者直推 git」的闭环只在 wiremock 上验过。
- **两处措辞待用户过目**:分区名「我分享的」/「我安装的」,以及无基线状态文案
  「没有获取记录,无法判断是否一致」(诚实但拗口)。⚠️ **后者在 v6 二期方案里会被整个
  删掉**(见「待处理」的 B),所以别单独花力气润色它。
...
5.3MB / exe.sig / app.tar.gz / app.tar.gz.sig),`latest` 公告牌 **三平台条目齐全且
各自带签名**(darwin-aarch64 / darwin-x86_64 / windows-x86_64),版本号 0.4.0;
macOS 包已签名·已公证·票据已装订;版本号 commit `26965ff` 双平台 CI 逐 job 绿。

⚠️ **发版日志里出现过一次 `failed to get run: ... connection reset by peer`**
(等 CI 那一步),脚本却继续往下走了——看起来像"把网络错误当成 CI 完成"。
**不要据此推断脚本有缺陷**:实测五个产物与三平台条目都在,那只是轮询中的一次失败
重试。但**下次发版看到这句话仍要自己 curl 核实一遍**,别信脚本自己那句"✅ 验收公告牌"
——如果它真的把错误吞了,后果是 Windows 同事永远停在旧版、查更新显示"已最新"。
...
推翻重做过,经过在 git log)。
**现役版本 v0.6.0**(2026-09-11 发布)。上一版 v0.5.0(2026-08-22)。
...
- 本机(2026-09-11,v0.6.0 发版前全量实测):Rust **900 passed / 0 failed / 3 ignored**
  (60 个 test binary,`./scripts/rust-test.sh full`,docker fixture 起着、live 真跑)
  + 前端 **995 passed / 55 files**;`clippy --all-targets` / eslint / tsc 干净。
  四条门控 live 手动跑过并真发了请求(耗时 63s / 35s / 26s / 0.35s ——
  **耗时本身就是"没走早退分支"的证据**)。
  ⚠️ 这个数字是**发版提交那一刻**的;Windows CI 随后红了(见上面 join 带斜杠一条),
  修复提交之后的数字以下一次实测为准。
- (历史)v6 二期收尾时:Rust **861 = 858 passed + 3 ignored**(54 个 test binary)+ 前端 **718 / 48 files**
  测试通过(2026-08-27 洁癖收尾时**串行**实测,五道闸全绿,
  clippy **--all-targets** / eslint / tsc 干净)。
  ⚠️ **这个数字复核过一次,过程值得记**:第一次重跑时红了两条(`e2e_author_loop` 与
  `gitea_live` 的清单断言),**两条都不在既有的"已知假红"清单里**,看起来像终审修复引入的回归;
  查清后是**两件与代码无关的事**——①**我自己并发跑了两个 `cargo test --workspace`**
  (共用 docker fixture 与 `target/`),②中断那些并发跑时在 fixture 库里**留下了 5 个测试技能目录**
  (`author-loop-*`/`conflict-live-*`/`share-live-*`),打红了 `gitea_live` 的清单断言。
  停掉并发 + 用 admin token 清掉残留之后,串行全量 **0 failed**,与修复波自报的数字逐字相同。
...
- **双平台 CI**:M9 全部八笔提交(任务 1–5 七笔 + 任务 6 一笔)**逐笔** `gh run view`
  ...
  的整体状态):
  `624056a`(任务1,31571731744)win 8m34s / mac 5m32s、
  `d8defab`(任务2,31574070318)win 8m16s / mac 5m27s、
  `00c5540`(任务3a,31577497361)win 7m57s / mac 5m18s、
  `349ac13`(任务3b,31579090118)win 8m12s / mac 4m55s、
  `0ed9e76`(任务4a,31581034961)win 8m8s / mac 4m12s、
  `a38a382`(任务4b,31582162813)win 8m34s / mac 4m12s、
  `b559dc6`(任务5,31585581383)win 8m6s / mac 5m3s、
  `79f8d8a`(任务6,31588094825)win 9m38s / mac 3m23s、
  `436de07`(任务6 修复,31589322735)win ✓ / mac ✓、
  `5db2602`(终审修复,31608087260)win ✓ / mac ✓
  ——**后两笔同样逐 job 核实**(2026-08-12,`gh run view --json jobs`),
  M9 十笔提交全部双平台绿。
  ⚠️ **这条记载本身有过一次真实的"整体状态当逐笔核实"复现**:初稿只对 `b559dc6`
  做了逐 job 核实,却写成"逐笔核实"——advisor review 与 coordinator review 先后
  指出后才把其余七笔全部补跑,结果确实全绿,但写下那句话的那一刻它并不成立。
  **别把这条自己犯过的错抄进下一份文档**。**M4 任务 1 的两笔(`3857720` / `a7a1de3`)
...
M11 新增的 IPC(更新日志,两条):`release_notes_state`(→ `{current, pending, all}`,
**只读**)、`release_notes_ack`(写 `config.lastSeenVersion`)。**刻意分成两条**:
显示即标记已读的话,用户升级后立刻退出就永远看不到了——写入由「关掉卡片」这个
动作触发。没有新事件、没有新错误码(读不到就是空列表,不报错)。
`config` 新增顶层可选字段 `lastSeenVersion`,不升 schemaVersion。
...
- **我的技能三区模型(v7)**——「我的技能」整页重画,动 `ownership.rs::in_builtin_library` /
  ...
  - **「在公司库里」的判据是三支之一成立**(`my_skills.rs::in_builtin_library`,
    ...
    直推的场景)∨ `remote_hash == local_hash`(名字撞上、没有记账,但内容逐字节
    相同)。**后两支的查询键都是 dir_slug**,所以设计决策 #3 说的"名字匹配"
    不是独立的第四个条件,而是隐含在"查得到"这件事本身里——查不到就是不匹配。
  ...
  - **多选组件四处统一**(`ToolPicker.tsx`,`ToolPickerItem` 是唯一的入参形状;
    ...
    **这是一处经用户看截图批准的画布偏离**:画布 §15 给确认条画的是 `list`,
    而 `list` 相对 `inline` 的**唯一**优势是"每行带路径",当时项目语境下那个路径
    按欠账③留空(`DetectedAgent` 只有全局 `globalSkillsDir`)。⚠️ **0.6.x 清掉
    欠账③之后这条理由已不成立**(路径可穿了),但拍板不变:维持 `inline` 是
    用户看过截图批的,理由换成真的——确认条这个语境 chip 流本身更合适,而
    `item.path` 两种布局都会渲染,填上路径就是把等宽串混进 chip 流。
    没有路径的 `list` 只是更高的 checkbox 竖排,正是它逼出了
    `max-h-[140px]` 滚动盒与"第一项被切一半",也就是用户 2026-09-10 真机投诉的
    那张图。🔴 **别照画布把它改回 `list`**——画布本体尚未更新,而"画布背离实现"
    与本项目复盘过的"实现背离画布"是同一个病的两面。原委见
    `docs/v7.6-共识.md`。⚠️ **`ProjectSections`**(项目行事后改选)仍是 `list`
    且 0.6.x 起**带路径**了,`list` 在那里的优势是真的;它是用户**没批过**的第五处,
    布局只登记不动)。两种 `layout`
    共用同一副 checkbox/label 骨架与类名,
    只是容器排布不同(`inline` 是 chip 流、`list` 是带路径的竖排清单)——
    "统一"指的是骨架,不是排布方向。排序"已勾在前、其余按注册表原相对顺序"
    在**拿到非空 `items`** 时算一次并用 `useRef` 惰性钉住,操作期间
    (含数据从探测中变为已知的那一刻)绝不重排;`useState(() => ...)` 的惰性
    初始值会在组件以 `items=[]` 挂载时把结果永久钉成空数组,这条坑已在
    组件注释与测试里钉住。0.6.x 起**新 items 与钉住的顺序毫无交集才重钉**
    (整批换掉时不再按旧顺序排;只增减几项时剩下的保持原位、新来的排末尾——
    按"集合变了就重钉"写过一版,真机复验当场回归);"换技能 = 换实例"仍是调用方的责任。
  - **项目级事后改选**(`project.rs::set_agents` + IPC `project_skill_set_agents`,
    任务 3/8):收的是**完整目标集合**(不是增量),内部按"扫这个项目里各工具目录、
    删多补缺"实现(`link_dirs` 建缺的、其余摘除),不碰 `skills-lock.json` 契约
    本身(项目级记账就是这份 lock,与全局 `state.installed` 不共用,详见下面
    「项目级安装(v5)」一节的既有约束)。`current_agents`(项目里这个技能眼下
    实际链到了哪些工具)是这条改选 UI **唯一的数据来源**——`DetectedAgent` 只有
    全局的 `globalSkillsDir`,答不出"这个项目"的问题,不接这条读通道,picker
    只能从"全不勾"起步、第一次点击就静默解除其余关联(直接撞铁律 7)。
    探测失败(`current_agents` 返回 `null`)时**不收窄候选**、也不能把已勾项
    悄悄清空——分两档:已勾项继续列(用最近一次成功探测留下的展示名),
    候选列表退化为"暂时读不到"的失败态文案,不是"没有可选工具"的假话。

  - 🔴 **分享目标按 `section` 分流,不按"有没有坐标"**(`my_skills`/`store/my-skills.ts`
    的 `shareTargetRepo`/`shareTargetRegistryId`,终审 C-1):「可分享到」区的行**恒指向
    公司(内建)技能库**——那个区里有大量从广场/GitHub 装来的技能,它们账上的坐标
    指着**外部来源**,按"有坐标就用账上坐标"这条直觉分流,用户点「分享」会把技能推去
    它自己的来源仓(多半没有写权限,或者更糟——真的推进了一个不相干的仓)。
    「已分享到」区才用账上坐标(那正是它当初分享去的地方)。
    **`ShareConfirm` 的路径预告与 `confirmShare` 的提交必须调同一个函数**:显示与提交
    分两处算就会出现"确认屏说推去 A、实际推去 B",而用户是照着那句话点的确认。
  - 🔴 **第 4 源(只在库里、本地没本体)只认内建库的作者信息**
    (`my_skills::builtin_library_attribution`,终审 C-2):这一档靠"库索引里这个技能的
    作者是不是我"把行造出来。用**任意源**的 attribution 就会把别的技能库(自定义源/
    广场仓)里恰好署我名的技能摆进公司库那三个区里——那是另一个库的技能,分享目标、
    更新去处全都对不上。查询键是 dir_slug,所以"名字撞上"这件事隐含在"查得到"里,
    不收窄到内建库就是在拿一个错的库回答"这是不是公司库里的技能"。
  - 🔴 **动作区(`SkillActionsBlock`)自己承载分享的成功与失败,并按 `dirSlug` 校验归属**
    (终审复审轮 1,C-A):`shareError` 因此从裸 `AppError` 改成
    `{ dirSlug, error }`(与 `shareDone` 对称,**刻意用类型而不是平行的
    `shareErrorFor` 字段**)。同一条道理的另外两处落点:`WhereBlocks::EachToolBlock`
    按 `toolFailuresFor` 过滤 `setAgents` 的失败;`LocalPanelBody` 自己接
    `useInstall.error`(那条路**没有** `InstallPanel` 兜着,商店那条路有)。
    **移除的部分失败刻意不摆进动作区**:`confirmRemove` 成功后 `load()` 会把这一行从
    `list` 里拿掉,而动作区挂在 `{skill && ...}` 之下,**块本身会卸载**——摆在那里最多
    闪一帧;它退化成"下次进「我的技能」页看到横幅"的延迟反馈,不是永久丢失;硬失败
    那条路另有 `RemoveDialog.removeError`(全局挂在 `App.tsx`)兜着。
    🔴 **分享结果的文案按 `flow` 分流,不按渲染那一刻的 `rowAction` 反推**
    (`lib/share-block.ts` 的 `ShareFlow`/`SHARE_DONE_LABEL`/`SHARE_FAILED_LABEL`,
    两个消费者共用同一张表):`shareDone`/`shareError` 各带一个
    `flow: "share" | "changes"`,由写入方(`confirmShare` / `runShareChanges`)当场记下。
    **反推是错的**——首次分享成功后 `load()` 会把这一行的 `relation` 从 `draft` 刷成
    `shared`、`section` 从 `shareable` 刷成 `sharedTo`,渲染那一刻已经看不出"刚才做的
    是哪件事";而 §12 把分享动作搬进动作区之后,**首次分享与推回改动落在同一个渲染点**,
    沿用「分享改动」那三个键就会对第一次分享的人说「**改动**已分享」。
    ⚠️ `useInstall.shareResult`(「保留并分享」)**不带 flow 也不需要**:它两条写入路径
    都走 `skill_share_changes`,恒是"推回改动"那一档——别顺手"统一"成同一张表。
    🔴 **动作区没有"没东西可摆就 return null"的早退,是有意删掉的**:那句
    `action.kind === "none" && items.length === 0` **不可达**(`localPresent` 为真时
    `buildRowMenuItems` 必产出「移除」,为假时 `rowAction` 必是 `pull`;全量 827 条
    前端测试下的运行时探针一次都没触发),留着就是空转模式 ①,而它最大的危害是
    **会兜住本该打红的注入信号**。哪天真出现空壳动作区,说明那两条前提之一变了,
    那时它是**真实分支**,该配测试正面处理,不是再补一道防御性早退。
...
- **装到项目是"选完再确认",不是"选完即装"**(2026-08-22 用户真机反馈后改):
  ...
  - **「最近的项目」省掉的只是"选文件夹"那一步,后续与手选完全一样**
    (2026-08-22 用户拍板,**推翻了同日早些时候"最近项目豁免确认"的设计**):
    一样进确认条、一样点「装到这里」才写盘。理由是**心流一致性**——同一个动作在
    两个入口有两种行为,用户每次都要先想"这次点下去会直接装吗"。
    一致性比省一次点击值钱。豁免的只是系统选择框,不是确认步骤。
  - **已装判定必须在"点之前"**:最近项目菜单里已装的标「已装」,陌生文件夹在
    确认条上直说。此前要等一整轮网络请求(下压缩包、建索引)才被告知"已经有了"。
    判据是**仓库目录名 `dirSlug`**,不是 lock 的 `key`——两者在广场技能里经常不同。
  - 🔴 **标出"已装"是知情,不是禁止**(2026-08-22 用户拍板:"装过的也能装,
    保留足够权利")。第一版把已装的菜单项做成 `disabled`、把确认条上的按钮撤掉,
    等于**把"已经装过"做成死路**。这是**过度套用了「不摆比解释好」**——那条针对的是
    "点了必然报错"的按钮;而重装完全合法:内容一样时它仍会**重建 agent 关联**
    (关联可能被别的工具删掉或改指),那正是用户想重装的理由。现在:
    已装的菜单项照样可点(点了与其余入口一样进确认条),确认条上主动作换成「覆盖重装」。
  - 🔴 **`force` 与 `confirmedReplace` 是两个开关,绝不能合并**
    (判定表在 `project::plan`,纯函数可单测,command 侧 match 穷尽):
    `force` = "我知道已经装了,再装一遍";`confirmedReplace` = "我已确认丢弃本地改动"。
    合并的后果是用户点一下「覆盖重装」就**静默抹掉自己改过的内容**(铁律 7 当场破)。
    所以 `force` 遇上"本体被改过"仍返回 `NeedDecision`,走既有的决策对话框。
    ⚠️ 另一侧同样要防:**`project_skill_update` 绝不能传 `force`**——传了的话
    "内容已经一样"那一档再也走不到,`AlreadyLatest`(已经是最新的)成了死代码,
    用户点「更新」会看到"已更新"而远端根本没有新内容。这条是实现时顺手改错、
    自己发现的,**没有任何测试拦得住它**(命令是薄壳),现在判定表上有正面断言。
  - **主按钮是终态时,作用域入口要显性化成文字按钮**(2026-08-22 用户反馈:
    "只有已启用和一个小三角,很难让人认为是可以继续安装到别的项目的")。
    `installed` 档的主按钮是 **disabled 的透明终态**(绿勾 +「已启用」),旁边只剩一个
    24px 纯图标 chevron,整块看起来就是"做完了"。**状态归状态,动作归动作**:
    那一档给 `InstallScopeMenu` 传 `label`,渲染成「装到项目… ⌄」文字按钮;
    主按钮本身是可点动作(获取/更新/替换)时保持图标,免得两个按钮抢注意力。
  - 🔴 **「已启用到 X、Y」是本次安装的临时态,打开详情时要收掉**
    (2026-08-22 用户反馈:"不然很割裂,目前只有重启才能回到已启用简易状态")。
    它说的是**这一次安装做了什么**,不是技能的属性——上个月装的技能打开就是
    简简单单一句「已启用」,刚装的却永远带着一段结果报告,同一个东西两种面孔。
    收在 `InstallPanel` 顶层的 effect 里(依赖只有 `dirSlug`),覆盖"关掉再打开"
    与"切走再切回"两条路。两条不能破的约束:
    - **只收终态(done/error)**:关详情面板时安装可能还在跑(running/choosing/
      conflict),清掉等于把进行中的流程整个丢掉;
    - **用 `useInstall.getState()` 读 phase,不订阅、不写进依赖**:写进依赖的话
      phase 一变成 done 就被这个 effect 立刻清掉,**结果报告一帧都看不到**。
      三条注入验证各自钉住一条。
  - 🔴 **项目安装的状态区(进行中/提示/待确认)在 `InstallPanel` 顶层渲染,
    与全局安装的 `phase` 无关**。此前它们挂在 `IdleFooter` 内部,于是"装完那一屏"
    (`phase === "done"`)里点最近项目,**确认条整个渲染不出来**——用户看得到入口、
    点下去却没反应。这类"入口在这一屏、反馈在另一屏"的错位,单测不特意跨 phase
    构造就发现不了(有测试专门钉住 done 档能出确认条)。
    同理 `DoneFooter` 也要摆「装到项目… ⌄」:装完那一屏原先零操作入口,
    想再装到项目只能关掉详情面板重开(2026-08-22 用户反馈)。
  - **确认条必须留一条"换个文件夹"**(同日拍板:"装到一个目录不应该没有任何
    可装到别的目录操作空间")。此前已装那一档只剩「取消」,是条死路。
...
- **升级后的更新日志(M11)有四条不变量,破一条就是可见缺陷或假话**
  ...
  - 🔴 **`RELEASE_NOTES.md` 必须打进安装包**(`bundle.resources`,**map 形式**
    `{"../RELEASE_NOTES.md": "RELEASE_NOTES.md"}`——裸数组的包内落点名要靠猜)。
    漏了它**只有安装包里没有那个文件**,dev 照常能读(`cargo build` 会把 resources
  ...
  - 🔴 **该显示哪几段的起点是「当前版本那一段」,不是文件开头**:发版纪律是
    **先写发版说明、脚本才肯发版**,所以"说明已进仓、包还没发"这段窗口里,文件里
    ...
    降级(记的版本比当前新)返回空,说"已更新到"是假话;**这条分支不是可选的**,
    去掉它切片会是 `notes[大..小]` 直接 panic(实测)。
  - 🔴 **`lastSeenVersion` 落 `config` 顶层,不进 `ui`**:`ui` 是 `Option` 且 `None`
    有独立语义(从未设置过外观 → 前端做 localStorage 一次性迁移)。放进去的话,
    第一次写"已看过"就会把 `ui` 物化成 `Some(默认值)`,**用户存在 localStorage 里的
    主题从此丢失**——一个与外观毫无关系的动作毁掉外观设置。也**不用**
    `app_update::ReadyState`:它在进程内、重启即作废,而"刚升级完"恰恰只在重启之后
    才成立;落盘还顺带覆盖了自更新之外的升级方式(同事直接发个新包过来)。
    缺席时靠 `ui.wizardDone` 区分"存量用户第一次升上来"(弹)与"全新安装"(不弹,
    对新人说「已更新到 x」是假话)。
  - **卡片挂商店页顶部,前提是 `store/ui.ts` 的 `page` 默认恒为 `store` 且不持久化**
    ——每次启动必定落在这一页。这条前提要是变了(比如将来记住上次停留的页),
    卡片就得跟着搬家。正文**必须限高可滚**(真机自查抓到:0.4.0 那段让卡片吃掉首屏
    近一半,而"打扰最低"正是选卡片而不是弹窗的理由)。
  - ⚠️ **`RELEASE_NOTES.md` 现在是用户可见文案的第三条通道**:i18n 守卫只扫 `src/`,
    `terminology.rs` 其余测试只扒 `AppError::new`。加守卫的当场就抓到四处
    (「整个代码仓库」+ 三处 ⚠️ emoji)。**往那个文件里写字要按用户可见文案的
    尺子来**,不再是开发者文档。
...
- **本体与位置模型(v6 二期)**——**这一节是本期的全部**,动 `converge.rs` /
  ...
  - 🔴 **core 备好的每一份"事实",界面必须真的有一处代码去读它**(整分支终审抓到的
    三条 Critical 全是这个形状,而九轮逐任务审查一条都没发现):`KeepReport` 的失败出口
    (`keep_version` 把 `trash_tree`/`unlink_dir`/`link_dir` 失败与 `canonical` 逐条收进返回值,
    **函数照常返回 `Ok`**——前端只 `await` 不取返回值的话,废纸篓被拒时是**零反馈死循环**:
    落选版本原样留着 → 刷新后仍是「有几个版本」→ 再点再失败)、`UninstallReport.unlinks`
    (移除时解链失败/跳过,**账已清、本体已进废纸篓、那一行从列表消失**,再没有入口能重试)、
    `shareBlocked`(算出来了但按钮不读它)。**判据:core 每加一个"收集起来由界面回报"的出口,
    同一笔提交里就要有一处代码读它,并有一条断言"那条失败真的到达了展示层"的测试。**
  - 🔴 **`share_installed`(分享改动/分享更新)与 `share()` 走同一把校验尺子**:
    A-2「不合格不让分享」是拍板不复议的硬规则,而**两条分享通道**都必须过
    `skills::validate_skill_dir`,且**排在任何网络请求之前**(测试断言请求条数 = 0)。
    界面侧三颗分享按钮同样按 `shareBlocked` 禁用 + 摆人话 + 给「打开文件夹」出口。
    ⚠️ 副作用**是有意的**:`name ≠ 目录名` 的存量技能(广场装的一部分、早年中文名分享的)
    从此「分享改动」点不动——**真机走查时别当回归**。
  - 🔴 **凡是会写盘的 core 编排函数,首行都要持 `watcher::app_write()` 守卫**:
    本期监听根从"只盯 canonical"扩到**canonical + 全部已存在的工具目录**,而新模型的本体
    **恰恰住在工具目录里**——`converge::set_agents` / `keep_version` / `share::share`
    不持守卫的话,自己写盘会触发监听 → `load()` **在动作进行到一半时替换整张列表**
    (勾在半途跳变、状态文案闪)。守卫清单由 `tests/watcher_guard.rs` 文本级钉住
    (**守名单不守新增**,加新写盘函数时要自己进名单)。
  - **措辞必须跟着语义走**:删除改成废纸篓之后,「无法找回」就成了假话;一档从"失败"
    变成"要你看一下"之后,汇总标题也得按内容分流。本期为此打回四次
    ——**改行为的那笔提交里就要把它的说法一起改**,别留给下一个人。
...
- 🔴 **分片只在写不下时才发生(2026-08-19 修)**——`auth::write_secret` 的顺序是
  **先试整份、失败才分片**,不是无条件分片。
  - 起因是真机体验缺陷:**macOS 钥匙串按「条目」逐个授权**,存成主条目 + 2 片就是
    3 个条目,用户**每次开应用被问三次「允许」**(启动路径 `App.tsx` → `auth_status`
    → `store.load()` 会把三个条目全读一遍)。
  - **分片是给 Windows 修的**(见下一条),macOS 钥匙串没有大小限制,在那里分片是
    **纯成本、零收益**。现在 macOS/Linux 落 1 个条目、弹 1 次;Windows 整份写失败后
    自动回退分片,行为与修之前完全一样。
  - **读取侧一行没改**:`parse_primary` 早就认得整份形式(那本是 v0.3.10 之前的格式,
    为兼容存量用户保留)——这次等于把"兼容旧格式"那条路变回主路。
  - **整份写失败一律回退,不匹配 `TooLong`**:各平台后端报什么错并不统一,
    匹配特定错误比"失败就换个写法"脆弱。
  - ⚠️ **存量凭证仍是分片格式**:改的是写入,已经存进去的不会自己变。
    **重新登录一次**才会落成单条目。不做启动时自动迁移——那要在启动路径上做
    写+删,失败面更大,而登录本来就是用户随时能做的一步。
  - ⚠️ **`pnpm dev` 每次编译后仍会问一次**:dev 是 **adhoc 签名**且 identifier 带
    编译 hash,钥匙串按签名身份记 ACL,二进制一变就认不出。**发布版是 Developer ID
    稳定签名,点一次「始终允许」之后不再问**。别把 dev 的表现当成发布版的缺陷。
  - 护栏:`core::auth` 三条纯逻辑测试(装得下只写一个条目 / 写不下回退分片且清单
    最后写 / 中途失败不留半份清单),两轮注入验证过。
    ⚠️ 假 writer 的上限**必须大于 `MAX_UTF16_UNITS_PER_CHUNK`**,否则分片本身也超限、
    回退那条路一样失败,测的就不是"回退成功"了(第一版真踩了这个,而且因为
    `head -15` 截断输出没看见它是红的——**统计测试结果永远别截断**)。
...
- 🔴 **登录是「源」级操作,不需要技能库坐标**(M9 终审抓到的跨任务缺陷,已修):
  ...
  必报错)——于是设置页那个「登录凭证」按钮**点了必然失败**,设计里"可选登录提配额"
  这条实际为零,匿名 60 次/时成了永久上限。现在 auth 链路走
  `registry::resolve_source(id)`(与 `resolve` 共用 `source_coords()`,只给
  kind/base_url、不要求 repo);**`resolve` 的 plaza 臂一个字没放宽**(那道守卫是对的)。
  - **这个缺陷的形状值得记住:两轮单任务审查各自都对,合起来错。** 任务 2 审查时
    判定"auth_* 对 plaza 报错是合理副作用"——当时还没有按钮,判断正确;任务 5 在其上
    摆了按钮,还写了句与事实相反的注释「广场同样支持它」。**逐任务审查在结构上
    看不见这类接缝,只有整分支终审能抓到**——别因为每个任务都绿就跳过终审。
  - **更该警惕的是:当时有一条测试把坏行为钉住了**。前端用例对着返回 `undefined` 的
    mock 断言"按钮渲染了"——渲染那一侧确实对,所以测试绿,而那条路**前后端都没有
    任何一条测试走过**(空转模式 #4)。修复的核心产物因此不是那个解析臂,是
    `session.rs::plaza_registry_id_walks_the_full_github_login_chain`
    (真走 resolve_source → 登录 → 断言凭证按 `plaza` 落进 store)
...
- **广场详情缓存刻意不记 head sha**(M9 终审裁决,设计 §2.2 原写"记 sha"已被推翻):
  ...
  每次命中多探一次 `branch_head` 是反向亏损;广场的典型用法是一次会话内的短时行为。
  **后果是详情可能短暂陈旧,但安装永远是新的**(安装走 `acquire` 独立路径重新拉)。
  代码处有注释挡着,**别"顺手补全"这个字段**。
...
- **广场搜索是"输入不发请求、回车才搜"**(2026-08-20 用户真机验收后改,
  原话:"按输入不跟手,有延迟"):
  - `store/plaza.ts` 拆成 `query`(输入框文本,改它零请求)与 `submittedQuery`
    (已提交的查询词,存 trim 后的值)。**`PlazaResults` 判空态必须用后者**
    ——用前者的话,用户刚敲第一个字热门榜就消失了。250ms 防抖已删除。
  - 🔴 **防连击的判据是"同词 + 搜索中"才忽略,不是"搜索中一律拦"**:
    连按回车搜同一个词发五个一样的请求纯属浪费;但**词一变必须立即以新的为准**
    (既有 `searchSeq` 序号比对照旧)。退化成按钮禁用或全局节流就等于用户明确
    反对过的"等待上次响应"。
  - **IME 回车是三重守卫**:`composingRef` + `isComposing` + **`keyCode === 229`**。
    前两个不够——WebKit(macOS 的 Tauri webview)在候选词上屏那一下 `compositionend`
    **先**发、确认键 `keydown` **后**到,那时两个标志都已是 false,中文用户"选个词"
    就会被当成按了搜索。**这条只有真机能证实**,jsdom 里只能造模拟事件顺序的用例。
  - **广场不该有公司技能库没有的顶栏控件**:守卫比的是两档 `aria-label` 集合相等
    (`Toolbar.test.tsx`),不写死某个按钮名——挡的是"两档 UI 割裂"这个类别,
    改文案不会让它失效。曾经加过一个「搜索」按钮,用户一眼看出"和设计割裂"。
  - ⚠️ **Tauri 的 `invoke` 没法真正 abort**:所谓"不等待上次响应"是**丢弃**旧结果,
    不是**取消**旧请求——它仍会在后台跑完。连点五次就是五个请求都在飞。
    界面表现无差别,但涉及配额时要记着这一点。
- **广场列表的"滚到底加载更多"是"一次取回、分批渲染",不是按需请求**
  (2026-08-19 追加,用户真机验收时提"没有复刻官网的拉到底自动增量加载"):
  - **上游没有分页**:`/api/search` 的 `offset` 参数实测**无效**(带与不带返回
    完全相同的 20 条),只能靠 `limit` 一次要更多。热门榜更直接——首页 SSR payload
    一次就给 **600 条**,此前 `truncate(24)` 把其余 576 条**直接扔了**。
  - 所以滚动加载**零新请求**:数据本来就在手里,`PlazaCardGrid` 只是分批往下渲染。
    **别把它写成"滚到底才去拉"**,那是假的。
  - `PLAZA_SEARCH_LIMIT` = **50**:实测 20 → 3.4KB/1.25s、50 → 8.5KB/**1.25s**、
    100 → 16.7KB/**2.12s**。50 是唯一不拿首屏速度换条数的档位,而用户上一轮抱怨的
    正是慢。**要再往上加之前先重测这三个数。**
  - `PLAZA_LEADERBOARD_LIMIT` 从"展示上限"变成 **sanity 上限**(防上游哪天返回巨量
    数据吃内存),不再是用户能看到的条数。
  - 🔴 **三条不变量,破一条就是可见缺陷**(实现在 `StorePage.tsx::PlazaCardGrid`):
    1. **`cards` 变化必须重置计数**——换搜索词、清空搜索框切回热门,都要从第一批
       重新开始。不重置的话,搜个新词会一上来就渲染"上次滚到的位置"那么多条。
       难被手工测试发现(很少有人滚到底之后立刻换词),有专门用例 + 注入验证钉住;
    2. **必须查 `entries[].isIntersecting`**:真实 `IntersectionObserver` 在
       `observe()` 那一刻就会用**当前**状态回调一次,哨兵还在首屏之下时那次是 false。
       不查就会一挂载白白多渲一批——**而 jsdom 里的替身不会自己触发,这种偏差本地
       测试看不见、只有真机才暴露**;
    3. **没有 `IntersectionObserver` 时降级为一次全渲染**,不是把剩下的条目永久藏起来
       ——界面上没有"加载更多"按钮可点,藏了就是永远看不到。
  - 测试替身在 `src/test/intersection-observer.ts`:**`disconnect()` 后必须真的不再
    触发**。组件每加载一批就重建观察者,替身若不把旧的摘掉,一次触发会打到新旧两个
    回调上、一次加载两批——**断言会漂而实现其实是对的**(替身自己制造的假象,
    是本项目吃过亏的假红温床)。
...
- **`installed_list` 返回的不等于 `state.installed`**(M4 任务 6a 立,v6 重写口径)。
  ...
  4. 只在技能库索引里、这台电脑没有本体的——**只有 `relation == Shared` 才摆**
     (换电脑 / 数据丢 / 绕过 app 直推)。
     🔴 **这一档点开详情要从库索引取内容**(`useLocalDetail.openFromLibrary`,
     `store_skill_detail`,不联网),不能走 `open({ dirSlug })`:那条路去读 canonical/<slug>,
     文件夹根本不存在,面板只剩「这个文件夹不是技能」,连「取回」都没有(2026-09-14
     Windows 真机,v6 起就在,一直没人在换过的电脑上点过详情)。
...
- **本地技能变更的三级刷新**(M4 任务 6c,0.6.x 起多一级兜底,见上一条):
  ...
  **加在 core 层而不是 commands 层**——scheduler 的自动更新不经过 commands,
  commands 不是真咽喉。写盘入口的名单以 `tests/watcher_guard.rs::GUARDED` 为准
  (M4 定的 acquire / acquire_batch / remove / create_skill 四个 + v6 二期终审 I-3 补的
  `converge::set_agents` / `converge::keep_version` / `share::share` 三个),别在这里抄第二份数字。
  判定放在防抖**吐出来的那一刻**(记录事件时还不知道这批是不是自己造的)。
  其余:监听根 = canonical + **每一个已经存在的**工具全局技能目录(v6 二期任务 5 起;
  此前只盯 canonical,而本体可以住在工具目录里之后那等于对最典型的编辑场景视而不见,
  原委见 `watcher.rs` 模块头「多根监听」);不存在的目录不建、不盯;
...
1. ~~**技能装到指定项目里**~~ —— **已完成**(v5 任务 1–6),见「现役机制约束」的
   ...
   (`6dcd97f`):广场技能装进项目要先幂等挂仓、项目级更新要带账上的库坐标。
2. ~~**应用更新后更新日志首屏显示,其他情况收敛到设置里**~~ —— **已完成**
   (M11 任务 1–4),见「现役机制约束」的「升级后的更新日志」一节。
   四个拍板点:日志**打进安装包**(零网络请求、离线可用)、形态是**商店页顶部
   可关闭卡片**(用户原选弹窗,听了推荐后改的:模态在这个应用里被留给"必须拍板
...
「我的技能」从三分区改成「我分享的/我安装的」两分区(该两分区又已被 v7 三区取代)。⚠️ **已随 v0.6.0 发布**,
`RELEASE_NOTES.md` 的 0.6.0 段落已写好。
...
**✅ v6(一期 + 二期)已随 v0.6.0 发布(2026-09-11)**。
...
- **搜索结果被过滤后没有补位**:排行榜是"先过滤后截断"(600 条丢 31 条,剩下的
  **整批**都留着给滚动加载,用户无感),搜索是"先按 `PLAZA_SEARCH_LIMIT` 取、再过滤",
  可见条数会**静默缩水**——`q=feishu` 实测(当时 limit=20)20 条全被丢,界面显示
  「没有匹配的技能」。这是"不摆比解释好"这条已拍板取舍的必然结果,不是缺陷;
  丢弃时有 `warn!` 记条数,排查时有线索。真要补位得**再发一次**更大 limit 的请求
  (上游没有分页,`offset` 无效),是新行为,不在 M10 范围。
  ⚠️ 2026-08-19 的追加改动把 limit 从 20 提到 50,**这只是把缩水的基数抬高,
  没有消除缩水**——别把它记成"已修"。
...
- ✅ **`.DS_Store` 会参与 `dir_content_hash`——已于 v6 二期任务 1 修复**
  (2026-08-24,原记载"M4 任务 6c 顺带发现,未修"已过期):`fsops::EXCLUDE_FILES`
  现为 `["metadata.json", ".DS_Store", "Thumbs.db", "desktop.ini"]`(访达/资源
  管理器三大平台各自的系统元文件一并排除)。原文这里提醒的"不要顺手改名单,
  要连带设计一次 hash 口径升级"正是这次的做法——名单是
  `dir_content_hash`(本地)与 `store::remote_content_hash`(索引缓存,建索引
  时用同一份 `is_excluded_rel`)共用的同一份,两侧同时改;口径升级用的是既有的
  `INDEX_SCHEMA_VERSION`(3→4)而不是另起一个 hash 版本号——版本不符直接丢弃
  重建缓存,不做"未知"这档过渡态,与该常量本身"缓存是可丢弃的派生数据"的既定
  取舍一致。**残留影响**:升级后首次打开商店会有一次全量缓存重建(预期行为,
  不是异常);但已装技能里**只有仓库归档本身真带了这几个系统文件的那一小撮**
...
- ~~**「关掉卡片 → 重启 → 不再弹」这条闭环未在真机上走过**~~ —— **用户已于
  ...
  **"关了下次不在"只有前端 mock 层覆盖**——真实 IPC → `acknowledge` → `save_config`
  ...
  缺席时起 `pnpm dev` → 卡片在 → 点右上角 × → 查 `~/.skillsync/config.json` 是否
  落成当前版本 → 重启 dev → 卡片应当不再出现。
  ⚠️ **这一步必须人工点**:GUI 自动化点击在本机被辅助功能权限挡下
  (osascript `click at` 报 -25208),盲点坐标会落到别的窗口上——试过一次,
  点到了系统登录窗口并把屏幕锁了。**别再用 osascript 盲点这个应用的界面。**
...
### 真机验收:agent 自主完成,不找用户当测试员(2026-09-24 起,0.7.0 首次实跑)

🔴 **用户拍板**:需要真机验收时,**自己设计用例、自己在真机上跑**,不要把"请你点一下 X
看看"甩给用户。只有两件事必须找用户,其余全部自主:
①**借一段屏幕时间**(computer-use 操作的就是用户正在用的那块屏幕,
没有后台/虚拟屏模式,一般 10 分钟够)——用一句话要,别把它说成"请你帮忙测";
②**钥匙串密码框**(输密码是禁止代劳的动作)——请用户输入并点「始终允许」,
同一 Developer ID 签名的包之后不再问。
🔴 **顺序是"上线前测,不是上线后测"**:App 自更新每 1 分钟查一次,macOS 下载即装,
发出去几分钟内全公司都在跑新版;而紧急修复一轮(公证 + CI + 上传)至少半小时。
"用户当测试员"的替代品不是"全公司当测试员"。

**1. 出待上线包(不发布、不改版本号)**
- 用发版那六行环境变量跑 `./scripts/build-release.sh`(Developer ID 签名 + 公证),
  产物在 `src-tauri/target/release/bundle/macos/SkillSync.app`。
- **版本号不动**(界面仍显示旧版本,这是刻意的):正式版发出后,用户 `/Applications` 里的
  旧版会按真实流程自更新,顺带把自更新那一跳也验了。测试包里升级日志卡片不显示新段,是预期。
- **从构建目录直接启动,不覆盖 `/Applications`**。启动前先**正常退出**已装的正式版
  (`osascript -e 'tell application id "com.dhslegen.skillsync" to quit'`——两版共用
  `~/.skillsync`,同时开会打架,见「本机环境」),再 `open -n <测试包路径>`,
  然后 `ps aux | grep "[S]killSync.app/Contents/MacOS"` **核实跑的是测试包那个路径**。
- ⚠️ 绝不拿 `pnpm dev` 的裸二进制做真机验收:用户明确要求"只能是正式 app",
  而且裸二进制拿不到 computer-use 授权。

**2. 用 computer-use 操作(实测过的坑,照着避)**
- `request_access(["SkillSync"])` 即可(属 full 档)。截图只显示已授权应用。
...
- 🔴 **Esc 送不到应用**(实测:连命令面板都关不掉,两次、清掉通知中心后仍然如此)。
  所以**凡是依赖 Esc 的行为在真机上验不了**,如实记"工具限制,未验证",
  由 jsdom 测试兜底——别记成通过,也别记成缺陷。
- 🔴 **每一次按回车之前先 `zoom` 看焦点框落在哪**。焦点多挪一格,「先不动」就变成
  「确认分享」。批量动作里也照做:`tab → zoom → 判断 → 再下一个 batch 回车`。

**3. 设计用例:从这一版改了什么倒推**
- 来源:`RELEASE_NOTES.md` 本版那一段(用户视角的承诺)+ 本地任务分解的 DoD。
  每条承诺至少一个用例,写成"做什么 → 应该看到什么"。
- 先把用例分成两类,**分类本身就是安全闸**:
  - **只读**(浏览、详情、预览、确认屏的预览轮、切主题)→ 真机 computer-use 跑;
  - **会写远端**(确认分享、覆盖、删除库里文件)→ 🔴 **绝不在真机上对公司技能库做**,
    只在 docker fixture 上跑(live 测试 `share_live` 等)。确认屏类用例在真机上只走到
    预览,然后回到「先不动」退出,**退出后核一眼那一行仍是原来的按钮**(= 零写入的证据)。
- 选有真实差异的数据:比如本地确实改过的自己的技能(那次用的是 code-annotator)。
  需要造场景(如"本体只在工具目录的无记账技能")时,只在 `~/.claude/skills` 之类
  **新建自己的临时目录**,验完删掉,绝不改用户已有的技能内容。

**4. 取证与记账**
- 每个结论都要有一张刚截的图撑着("看了再断言",别凭上一张图或 DOM 记忆推);
  小字用 `zoom`。结果记成表:✅ 通过 / ❌ 缺陷 / ⚪ 验不了(写原因)。
- 🔴 **看清是哪一张卡片再下结论**:那次把一张被窗口裁掉底栏的卡片,当成了它上面
  那张卡片的"1 个月前",差点报了一个不存在的"卡片与详情时间不一致"缺陷。
- 与 GUI 并行跑不占屏幕的:`./scripts/rust-test.sh full` + 四条门控 live + 逐 job 核 CI。

**5. 之后**
- 全绿 → `publish-release.sh`;curl 核终态 → 退出测试包、启动 `/Applications` 旧版,
  看它自更新到新版(自更新整跳的真机验证)。
- 真机发现缺陷 → 修 → 重新出测试包 → 只重跑受影响的用例 + 冒烟一遍。
...
**发版前也要手动跑一次真实系统废纸篓 live 测试**(v6 二期任务 1 加,2026-08-24):

```
cd src-tauri && SKILLSYNC_TRASH_LIVE=1 cargo test --test trash_live -- --nocapture
```

理由同上一条:`core::fsops::SystemTrash` 调的是操作系统原生废纸篓 API,单测的
`SandboxTrash` 只在临时目录内部模拟"移动",验证不了真实调用能不能成功
——这条默认跳过,"测试全绿"对它同样零信息量。
...
- 🔴 **任何两个测试进程都别并发跑**——不只是 vitest 与 cargo test,**两个 `cargo test --workspace`
  ...
  会互相踩出**看起来像真回归的假红**(当时红的是 `e2e_author_loop` 与 `gitea_live` 的清单断言,
  而它们**都不在既有的"已知假红"清单里**,差点被当成终审修复引入的回归)。
  **判据:开跑前先 `pgrep -f "cargo test"` 确认没有在跑的;要复核别人报的计数,
  先停掉一切在跑的测试再串行重跑。**
- vitest 与 cargo test **别并发跑**:互抢 CPU 会让 vitest 的 5 秒超时用例偶发假红
  (实测 environment 阶段被拖到 566s)。
- 🔴 **`pgrep -f "cargo test"` 写进一个"等到没有再继续"的循环里会匹配到循环自己**
  (v7 任务分解 R14,2026-08-28 控制者亲自踩到):`-f` 是按**完整命令行**匹配,
  而 `until ! pgrep -f "cargo test"; do sleep 1; done` 这条循环命令的文本里本身就含
  "cargo test" 这个子串,于是这个判据**永远为真、循环永不退出**。上面那条"开跑前
