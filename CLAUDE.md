...
## 当前进度(2026-08-11,M7 完成 + M8 任务 1-3 完成,现役 v0.3.12)
...
推翻重做过,经过在 git log)。
**现役版本 v0.3.12**(2026-08-11 发布,公告牌实测指向它:三平台条目齐全,
从发布仓下载的 exe 用公告牌里的签名 + 打进包的公钥 minisign 验签通过)。
**v0.3.10 是首个含 Windows 包的版本**;**v0.3.11 修 Windows 登录失败**
(凭据管理器大小上限,见「关键事实」的钥匙串分片条目);
**v0.3.12 = Windows 自更新"下载好再等用户点" + 三个交互修复**
(拖窗口 / 程序坞图标 / 托盘左右键)。0.3.1–0.3.7 是**为了验证自更新链路连发的**
...
- 本机:Rust 481 + 前端 411 测试通过(M8 任务 3 起),clippy(**--all-targets**)/eslint/tsc 干净,
...
> **v0.3.12 已于 2026-08-11 发出**(含"下载好再等用户点"的修复),条件到此备齐
> ——装着 0.3.10/0.3.11 的 Windows 机器现在能升到它,但**这一跳有没有跑通仍未知**,
> 要用户在真机上看:pill 出现 → 点了才装 → 应用不再自己退出。
