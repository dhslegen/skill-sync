...
import { cardState, isMine, localProbeOf, remoteHashOf, type LibraryRef, type MeRef } from "./update";
...
describe("cardState", () => {
  ...
  it("没装过、磁盘上也没有 → install", () => {
    ...
    expect(cardState(undefined, "sha:remote", undefined, { known: true })).toBe("install");
  });

  it("🔴 有记账,但**探过了**磁盘上没有本体 → 仍是 install,不是 onDisk", () => {
    // 这是本次拍板的核心推论:入口从"问账本"改成"问磁盘"。装了又被手动删掉
    // 目录的技能,从「已启用」变成「获取」——正确,不是回归(Q47-A)。
    //
    // ⚠️ 必须显式传 `{ known: true }`:v7.6 复审之后,**省略这个形参的含义是
    // 「还没探到」而不是「磁盘上没有」**,那一档会用记账当代理答 onDisk
    // (见下面那个 describe)。这两件事的后果恰好相反。
    expect(cardState({ contentHash: "sha:same" }, "sha:same", undefined, { known: true })).toBe(
      "install",
    );
  });

  it("装了且指纹一致 → onDisk(v7.6 起 installed 并入 onDisk,不再是独立档)", () => {
    expect(cardState({ contentHash: "sha:same" }, "sha:same", undefined, { known: true, hash: "sha:same" })).toBe("onDisk");
  });

  it("装了但远端指纹不同、本地没改(localHash 与记账基线一致)→ update", () => {
    expect(cardState({ contentHash: "sha:old" }, "sha:new", undefined, { known: true, hash: "sha:old" })).toBe("update");
  });
  ...
  it("🔴 v7.6:「更新」比以前更准——不同 + 有账 + localHash 与记账基线不同(用户改过)→ differs,不是 update", () => {
    ...
    expect(cardState({ contentHash: "sha:old" }, "sha:new", undefined, { known: true, hash: "sha:local-edit" })).toBe(
      "differs",
    );
  });

  it("有账时 remoteHash 缺失(索引没有这个技能的指纹)→ onDisk 处理(宁可漏报)", () => {
    expect(cardState({ contentHash: "sha:old" }, "", undefined, { known: true, hash: "sha:old" })).toBe("onDisk");
  });
  ...
  it("同名技能来自另一个库 → otherLibrary,不是 update", () => {
    ...
    expect(cardState(record, "sha:whatever", library, { known: true, hash: "sha:design" })).toBe("otherLibrary");
  });
  ...
  it("广场坐标:已装且指纹一致 → onDisk", () => {
    ...
    expect(cardState(record, "sha:same", library, { known: true, hash: "sha:same" })).toBe("onDisk");
  });
  ...
  it("广场坐标:索引指纹不等、本地未改 → update", () => {
    ...
    const library: LibraryRef = { registryId: "plaza", owner: "vercel-labs", repo: "skills" };
    expect(cardState(record, "sha:new", library, { known: true, hash: "sha:old" })).toBe("update");
  });
  ...
  it("广场坐标:同名技能装自另一个技能库(不同 owner/repo) → otherLibrary", () => {
    ...
    const library: LibraryRef = { registryId: "plaza", owner: "vercel-labs", repo: "skills" };
    expect(cardState(record, "sha:whatever", library, { known: true, hash: "sha:whatever" })).toBe("otherLibrary");
  });
  ...
  it("🔴 v7.6 行为翻转:装了、只有本地改过(远端没变)→ differs,不再是「已启用」", () => {
    ...
    expect(cardState(record, "sha:same", undefined, { known: true, hash: "sha:local-edit" })).toBe("differs");
  });

  it("远端有新版、本地没改(localHash 与记账基线一致)→ update", () => {
    const record = { contentHash: "sha:old", localModified: false };
    expect(cardState(record, "sha:new", undefined, { known: true, hash: "sha:old" })).toBe("update");
  });
  ...
  it("远端有新版、本地也确实改了(localHash 与记账基线不同)→ differs,不是 update", () => {
    ...
    const record = { contentHash: "sha:old", localModified: true };
    expect(cardState(record, "sha:new", undefined, { known: true, hash: "sha:local-edit" })).toBe("differs");
  });
  ...
  it("otherLibrary 判定排在按内容指纹比对之前(与 core acquire::precheck 判定顺序一致)", () => {
    ...
    const library: LibraryRef = { registryId: "company", owner: "skills", repo: "skills" };
    expect(cardState(record, "sha:design", library, { known: true, hash: "sha:design" })).toBe("otherLibrary");
  });
  ...
  it("无记账 + 本机有本体 + 指纹相同 → onDisk", () => {
    expect(cardState(undefined, "sha:same", undefined, { known: true, hash: "sha:same" })).toBe("onDisk");
  });

  it("无记账 + 本机有本体 + 指纹不同 → differs(v7.6 前叫 onDiskDiffers)", () => {
    expect(cardState(undefined, "sha:remote", undefined, { known: true, hash: "sha:local" })).toBe("differs");
  });

  it("无记账 + 本机有本体 + remoteHash 为空(广场)→ onDisk(漏报是刻意的,Q43-A)", () => {
    // 广场卡片没有内容指纹,`remoteHash` 恒传空串——哪怕本地内容其实不同,
    // 也不该说「与库里不同」(那需要一个不存在的比对基准)。
    expect(cardState(undefined, "", undefined, { known: true, hash: "sha:local" })).toBe("onDisk");
  });

  it("localHash 本身是空串(有本体但指纹读不到)时同样按'相同'处理", () => {
    expect(cardState(undefined, "sha:remote", undefined, { known: true, hash: "" })).toBe("onDisk");
  });
...
describe("localProbeOf", () => {
  it("🔴 list 为 null/undefined = 「还没探到」,不是「探过、没有」", () => {
    // 这一条是 v7.6 复审修的那个缺陷的正面守卫。上一版(`localHashOf`)把
    // "还没加载"与"磁盘上没有"都返回 undefined,于是 `useMySkills.load()`
    // 失败时,满屏**确实在电脑上**的技能会永久显示「获取」。
    expect(localProbeOf(null, "weekly-report")).toEqual({ known: false });
    expect(localProbeOf(undefined, "weekly-report")).toEqual({ known: false });
  });

  it("list 加载过但找不到这一行 = 「探过了,磁盘上没有」", () => {
    expect(localProbeOf([], "weekly-report")).toEqual({ known: true, hash: undefined });
  });

  it("只取 localPresent 的行,不是随便一条匹配 dirSlug 的记录", () => {
    const list = [{ dirSlug: "weekly-report", localPresent: false, localHash: "sha:x" }];
    expect(localProbeOf(list, "weekly-report")).toEqual({ known: true, hash: undefined });
  });

  it("localPresent 为真时给出它的 localHash(哪怕是空串)", () => {
    const list = [{ dirSlug: "weekly-report", localPresent: true, localHash: "" }];
    expect(localProbeOf(list, "weekly-report")).toEqual({ known: true, hash: "" });
    const list2 = [{ dirSlug: "weekly-report", localPresent: true, localHash: "sha:x" }];
    expect(localProbeOf(list2, "weekly-report")).toEqual({ known: true, hash: "sha:x" });
  });
});

describe("🔴 v7.6 复审:磁盘的答案还没到时,不做任何关于磁盘的断言", () => {
  it("未探到 + 有账 + 指纹一致 → onDisk(用记账当代理,不是「获取」)", () => {
    expect(cardState({ contentHash: "sha:same" }, "sha:same", undefined, { known: false })).toBe(
      "onDisk",
    );
  });

  it("未探到 + 有账 + 库里前进了 → update", () => {
    expect(cardState({ contentHash: "sha:old" }, "sha:new", undefined, { known: false })).toBe(
      "update",
    );
  });

  it("未探到 + 无账 → install(没有任何证据说它在电脑上)", () => {
    expect(cardState(undefined, "sha:new", undefined, { known: false })).toBe("install");
  });

  it("形参省略时默认「未探到」——绝不能默认成「探过、没有」", () => {
    expect(cardState({ contentHash: "sha:same" }, "sha:same")).toBe("onDisk");
  });
});
